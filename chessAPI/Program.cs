...
try
{
    ...
    #region "Team REST Commands"

    app.MapGet("team/{id}", async (ITeamBusiness bs, long id) =>
    {
        var x = await bs.getTeam(id).ConfigureAwait(false);
        return x != null ? Results.Ok(x) : Results.NotFound();
    });

    app.MapPost("team",
    [AllowAnonymous] async (ITeamBusiness bs, clsNewTeam newTeam) =>
    {
        await bs.startTeam(newTeam).ConfigureAwait(false);
        return Results.Ok();
    });
